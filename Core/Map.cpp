...
Tile::Tile(): tileType(EMPTY), tileDirection(NONE){}

Map::Map(int map) :tiles(vector(HEIGHT, vector<Tile>(WIDTH))){
    // 加载ini
    QSettings settings(QString(":/res/Game/Path/p%1.ini").arg(map), QSettings::IniFormat);
